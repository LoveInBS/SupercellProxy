...
public class Proxy(string upstreamHost, int upstreamPort, int listenPort)
{
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            _ = HandleClientAsync(client, upstreamHost, upstreamPort, cancellationToken);
        }
    }
    ...
    private static async Task HandleClientAsync(TcpClient client, string upstreamHost, int upstreamPort, CancellationToken cancellationToken = default)
    {
        ...
        await using var c2s = clientConn.GetStream();
        await using var s2c = upstream.GetStream();

        try
        {
            await Task.WhenAll(PumpAsync(c2s, s2c, Direction.Serverbound, cancellationToken), PumpAsync(s2c, c2s, Direction.Clientbound, cancellationToken));
        }
        catch (Exception exception)
        {
